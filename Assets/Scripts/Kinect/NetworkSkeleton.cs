...
public class NetworkSkeleton : MonoBehaviour {
    ...
    List<KinectSkeletonRecord> skeletons = new List<KinectSkeletonRecord> ();
    ...
    */
    ...
    void Update () {
        ...
        foreach (KinectSkeletonRecord skeleton in skeletons) {
        ...
        lock (_skeleslock) { 
            ...
            foreach(NetworkSkeletonInfo sk in skeles) {
                ...
                KinectSkeletonRecord existingKinectSkeleton = skeletons.FirstOrDefault (skeleton => skeleton.root?.name == bodyId.ToString ());

                // Recognises this skeleton
                if (existingKinectSkeleton != null) {
                    ApplyJointDataToNetworkSkeleton (sk.joints, existingKinectSkeleton);
    ...
    void ApplyJointDataToNetworkSkeleton (NetworkJoint[] joints, KinectSkeletonRecord realKinectSkeleton) {
        // Do joint moves
        for (JointId i = 0; i < JointId.Count; i++) {
            Vector3 pos = joints[(int)i].position;
            pos.y *= -1;
            pos *= 0.001f;
            
            realKinectSkeleton.joints[i].transform.localPosition = pos;
            realKinectSkeleton.joints[i].transform.localRotation = joints[(int)i].rotation;
        }
        realKinectSkeleton.lastUpdateTime = Time.time;
        realKinectSkeleton.lastPosition = joints[0].position;
    }
    ...
    void CreateDebugSkeletons (string rootName) {
        ...
        for (JointId joint = 0; joint < JointId.Count; joint++)
        {
            var cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            
            // name of the joint
            var t_JointName = Enum.ToObject(typeof(JointId), (int)joint).ToString();
            
            cube.name = t_JointName;
            cube.transform.localPosition = Vector3.zero;
            cube.transform.localScale = Vector3.one * 0.05f;
            cube.transform.parent = skeletonRoot.transform;
            joints.Add(joint, cube);
        }
        ...
        skeletons.Add(newKinectSkeleton);
