...
public struct SkeletonInfo
{
    ...
    public Joint GetRawJoint(int joint)
        => skeleton.GetJoint(joint);
}
    ...
    void kinectTask()
    {
        using (Device device = Device.Open(0))
        {
            ...
            using (Transformation transform = device.GetCalibration().CreateTransformation())
            {
                ...
                try
                {
                    using (Tracker tracker = Tracker.Create(c,
                            ...
                            using (Frame frame = tracker.PopResult(TimeSpan.Zero, throwOnTimeout: false))
                            {
                                ...
                                lock (m_bufferLock)
                                {
                                    ...
                                    for (int body = 0; body < frame.NumberOfBodies; body++)
                                        m_currentSkeletons.Add(new SkeletonInfo(frame.GetBodySkeleton((uint)body),
